...
public class FarmManager {
    ...
    protected void addToInventory(String productName) {
        try {
            switch (productName) {
                case "egg" -> farm.stockProduct(Barcode.EGG, Quality.REGULAR);
                case "milk" -> farm.stockProduct(Barcode.MILK, Quality.REGULAR);
                case "jam" -> farm.stockProduct(Barcode.JAM, Quality.REGULAR);
                case "wool" -> farm.stockProduct(Barcode.WOOL, Quality.REGULAR);
                default -> {
            ...
            shop.displayProductAddFailed(e.getLocalizedMessage());
            return;
        }
    ...
    protected void addToInventory(String productName, int quantity) {
        try {
            switch (productName) {
                case "egg" -> farm.stockProduct(Barcode.EGG, Quality.REGULAR, quantity);
                case "milk" -> farm.stockProduct(Barcode.MILK, Quality.REGULAR, quantity);
                case "jam" -> farm.stockProduct(Barcode.JAM, Quality.REGULAR, quantity);
                case "wool" -> farm.stockProduct(Barcode.WOOL, Quality.REGULAR, quantity);
                default -> {
                    shop.displayProductAddFailed(productName);
                    return;
                }
    ...
    protected void createCustomer() {
        ...
        String address = "";
        try {
            name = shop.promptForCustomerName();
            number = shop.promptForCustomerNumber();
            address = shop.promptForCustomerAddress();
    ...
    protected void initiateTransaction(String transactionType) {
        ...
        try {
            customer = farm.getCustomer(name, number);
        ...
        switch (transactionType) {
            ...
            case "-categorised", "-c" -> transaction = new CategorisedTransaction(customer);
